...
const WORKOUT_PROGRAM = {
  0: { name: 'Tam Dinlenme', emoji: '😴', exercises: [], isRestDay: true },
  1: { name: 'Yoğun Göğüs, Biceps & Karın', emoji: '🔥', exercises: getExercises('dumbbell_flat_bench_press', 'push_up', 'dumbbell_pull_over', 'concentrated_bicep_curl', 'dumbbell_preacher_curl', 'tuck_crunch', 'hanging_knee_raises', 'plank'), isRestDay: false },
...
function renderHistoryScreen() {
  ...
  if (weights.length > 0) {
    latestWeight = weights[weights.length - 1].weight + ' kg';

    // Sort weights newest first
    const sortedWeights = [...weights].sort((a, b) => new Date(b.date) - new Date(a.date));
    weightHistoryHTML = sortedWeights.slice(0, 3).map(w => {
      const d = new Date(w.date);
      return `<span style="display:inline-block; font-size: 0.8rem; background: rgba(255,255,255,0.05); padding: 4px 8px; border-radius: 4px; border: 1px solid var(--glass-border); margin-right: 6px;">
          ${w.weight}kg (${d.getDate()}/${d.getMonth() + 1})
        </span>`;
    }).join('');
  }
