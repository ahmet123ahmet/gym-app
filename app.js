...
const CHALLENGES_CONFIG = [
  {
    ...
    color: '#ff6b35',
    colorRgb: '255, 107, 53',
    type: 'timer',   // stopwatch — shows MM:SS
    unit: 'süre',
    description: 'Vücudunu düz tut, ne kadar dayanabilirsin?',
    tip: 'Nefesini düzenli al, kalçanı ne yukarı kaldır ne de düşür.',
    image: './images/plank.jfif'
  },
  {
    id: 'ip_atlama',
    name: 'İp Atlama',
    ...
    color: '#a855f7',
    colorRgb: '168, 85, 247',
    type: 'reps',     // input — enter number of jumps
    unit: 'atlayış',
    description: 'Hiç takılmadan kaç kere atlayabilirsin?',
    tip: 'Ritimli ve hafif zıpla, bileğini kullan.',
    image: './images/ip-atlama.png'
  },
  {
    id: 'sinav',
    name: 'Şınav',
    ...
    color: '#2ecc71',
    colorRgb: '46, 204, 113',
    type: 'reps',     // input — enter number of push-ups
    unit: 'tekrar',
    description: 'Durmadan kaç şınav çekebilirsin?',
    tip: 'Göğsünü yere yakın getir, sırtını düz tut.',
...
function getChallengeRecords() {
  return JSON.parse(localStorage.getItem('gym_challenge_records') || '{}');
}

function getChallengeHistory() {
  return JSON.parse(localStorage.getItem('gym_challenge_history') || '[]');
}

// For timer challenges: value = total seconds (integer)
// For reps challenges: value = integer count
function formatChallengeValue(challenge, value) {
  if (challenge.type === 'timer') {
    const m = Math.floor(value / 60);
    const s = value % 60;
    return `${m > 0 ? m + ' dk ' : ''}${s} sn`;
  }
  return `${value} ${challenge.unit}`;
}

function saveChallengeResult(challengeId, value) {
  // value: seconds (timer) or count (reps)
  const records = getChallengeRecords();
  const history = getChallengeHistory();

  const current = records[challengeId];
  const isNewRecord = !current || value > current.value;

  records[challengeId] = isNewRecord ? { value, date: getDateKey(new Date()) } : current;
  localStorage.setItem('gym_challenge_records', JSON.stringify(records));
  ...
  localStorage.setItem('gym_challenge_history', JSON.stringify(history));
...
function renderChallengesScreen() {
  ...
  const cards = CHALLENGES_CONFIG.map(ch => {
    const rec = records[ch.id];
    const recDisplay = rec
      ? formatChallengeValue(ch, rec.value)
      : '—';
    const recDate = rec ? rec.date : null;
    const recDateStr = recDate
      ? (() => { const d = new Date(recDate + 'T00:00:00'); return `${d.getDate()} ${MONTH_NAMES[d.getMonth()]} ${d.getFullYear()}`; })()
      : '';

    return `
      <div class="challenge-card" onclick="openChallengeModal('${ch.id}')" style="--ch-color: ${ch.color}; --ch-rgb: ${ch.colorRgb};">
        <div class="challenge-card-glow"></div>
        <div class="challenge-card-top">
          <div class="challenge-icon">${ch.emoji}</div>
          <div class="challenge-info">
            <div class="challenge-name">${ch.name}</div>
            <div class="challenge-desc">${ch.description}</div>
          </div>
        </div>
        <div class="challenge-record-box">
          <div class="challenge-record-label">🏆 Kişisel Rekor</div>
          <div class="challenge-record-value" style="color: ${ch.color}">${recDisplay}</div>
          ${recDateStr ? `<div class="challenge-record-date">${recDateStr}</div>` : ''}
        </div>
        <div class="challenge-card-footer">
          <span class="challenge-start-hint">Yeni Deneme Başlat →</span>
        </div>
      </div>
    `;
  }).join('');
  ...
  container.innerHTML = `
    <div style="padding-top: 8px;">
      ...
      <div style="font-size: 0.82rem; color: var(--text-muted); margin-bottom: 20px;">Kendi rekorunu kır!</div>
      <div class="challenges-list">${cards}</div>
      <button class="btn-secondary" style="margin-top: 20px; display: flex; align-items: center; justify-content: center; gap: 8px;" onclick="showChallengeHistory()">
        📋 Tüm Geçmiş Denemeler
      </button>
    </div>
  `;
}
...
function openChallengeModal(challengeId) {
  ...
  const ch = CHALLENGES_CONFIG.find(c => c.id === challengeId);
  const overlay = document.getElementById('challenge-modal-overlay');
  const modal = document.getElementById('challenge-modal-content');

  let bodyHtml = '';

  if (ch.type === 'timer') {
    bodyHtml = `
      <div class="challenge-timer-display" id="ch-timer-display">00:00</div>
      <div class="challenge-timer-controls">
        <button class="ch-ctrl-btn ch-ctrl-start" id="ch-start-btn" onclick="toggleChallengeTimer()">
          ▶ Başlat
        </button>
        <button class="ch-ctrl-btn ch-ctrl-reset" onclick="resetChallengeTimer()">↺ Sıfırla</button>
      </div>
      <button class="btn-primary" style="margin-top: 16px;" onclick="saveTimerChallenge()" id="ch-save-btn" disabled>
        ✅ Kaydet
      </button>
    `;
  } else {
    bodyHtml = `
      <div style="font-size: 0.9rem; color: var(--text-secondary); text-align: center; margin-bottom: 16px;">Durmadan yaptığın tekrar sayısını gir:</div>
      <div class="challenge-reps-input-wrap">
        <button class="ch-rep-adj" onclick="adjustReps(-1)">−</button>
        <input type="number" id="ch-reps-input" class="challenge-reps-input" value="0" min="0" placeholder="0">
        <button class="ch-rep-adj" onclick="adjustReps(1)">+</button>
      </div>
      <div style="font-size: 0.8rem; color: var(--text-muted); text-align: center; margin-top: 8px;">${ch.unit}</div>
      <button class="btn-primary" style="margin-top: 20px;" onclick="saveRepsChallenge()">
        ✅ Kaydet
      </button>
    `;
  }
  ...
  modal.innerHTML = `
    <div class="modal-header">
      <div>
        <div style="font-family: 'Outfit', sans-serif; font-size: 1.4rem; font-weight: 800; color: ${ch.color};">${ch.emoji} ${ch.name}</div>
        <div style="font-size: 0.82rem; color: var(--text-muted); margin-top: 2px;">💡 ${ch.tip}</div>
      </div>
...
function closeChallengeModal() {
  stopChallengeTimer();
  document.getElementById('challenge-modal-overlay').classList.remove('active');
...
function toggleChallengeTimer() {
  if (challengeTimerRunning) {
    ...
    if (btn) { btn.textContent = '▶ Devam Et'; btn.classList.remove('ch-ctrl-pause'); btn.classList.add('ch-ctrl-start'); }
  } else {
    // start
    challengeTimerRunning = true;
    const btn = document.getElementById('ch-start-btn');
    if (btn) { btn.textContent = '⏸ Durdur'; btn.classList.remove('ch-ctrl-start'); btn.classList.add('ch-ctrl-pause'); }
    challengeTimerInterval = setInterval(() => {
      challengeElapsedSeconds++;
      updateChallengeTimerDisplay();
      // enable save button once started
...
function resetChallengeTimer() {
  ...
  updateChallengeTimerDisplay();
  const btn = document.getElementById('ch-start-btn');
  if (btn) { btn.textContent = '▶ Başlat'; btn.classList.remove('ch-ctrl-pause'); btn.classList.add('ch-ctrl-start'); }
  const saveBtn = document.getElementById('ch-save-btn');
  if (saveBtn) saveBtn.disabled = true;
}

function updateChallengeTimerDisplay() {
  const el = document.getElementById('ch-timer-display');
  if (!el) return;
  const m = Math.floor(challengeElapsedSeconds / 60);
  const s = challengeElapsedSeconds % 60;
  el.textContent = `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;
}

function saveTimerChallenge() {
  if (challengeElapsedSeconds === 0) { showToast('Önce zamanlayıcıyı başlat!'); return; }
  stopChallengeTimer();
  const ch = CHALLENGES_CONFIG.find(c => c.id === activeChallengeId);
  const isRecord = saveChallengeResult(activeChallengeId, challengeElapsedSeconds);
  closeChallengeModal();
  const display = formatChallengeValue(ch, challengeElapsedSeconds);
  if (isRecord) {
    showNewRecordToast(ch.name, display);
  } else {
    showToast(`Kaydedildi: ${display}`);
  }
  renderChallengesScreen();
}
...
function adjustReps(delta) {
  const inp = document.getElementById('ch-reps-input');
  if (!inp) return;
  const cur = parseInt(inp.value) || 0;
  inp.value = Math.max(0, cur + delta);
}

function saveRepsChallenge() {
  const inp = document.getElementById('ch-reps-input');
  const val = parseInt(inp ? inp.value : 0) || 0;
  if (val <= 0) { showToast('Lütfen geçerli bir sayı gir!'); return; }
  const ch = CHALLENGES_CONFIG.find(c => c.id === activeChallengeId);
  const isRecord = saveChallengeResult(activeChallengeId, val);
  closeChallengeModal();
  const display = formatChallengeValue(ch, val);
  if (isRecord) {
    showNewRecordToast(ch.name, display);
  } else {
    showToast(`Kaydedildi: ${display}`);
...
function showNewRecordToast(name, display) {
  const toast = document.getElementById('toast');
  toast.innerHTML = `🏆 Yeni Rekor! ${name}: ${display}`;
  toast.style.background = 'linear-gradient(135deg, #ff6b35, #ff3c3c)';
  toast.classList.add('show');
  setTimeout(() => {
...
function showChallengeHistory() {
  ...
  const modal = document.getElementById('challenge-history-content');

  let rows = '';
  if (history.length === 0) {
    rows = `<div class="empty-state" style="padding: 30px 0;"><span class="empty-emoji">📭</span><div class="empty-title">Henüz deneme yok</div><div class="empty-text">İlk meydan okumana başla!</div></div>`;
  } else {
    rows = history.map(entry => {
      const ch = CHALLENGES_CONFIG.find(c => c.id === entry.challengeId);
      if (!ch) return '';
      const d = new Date(entry.date + 'T00:00:00');
      const dateStr = `${d.getDate()} ${MONTH_NAMES[d.getMonth()]} ${d.getFullYear()}`;
      const display = formatChallengeValue(ch, entry.value);
      return `
        <div class="challenge-history-row">
          <div class="challenge-history-left">
            <span style="font-size: 1.3rem;">${ch.emoji}</span>
            <div>
              <div class="challenge-history-name">${ch.name} ${entry.isRecord ? '<span class="ch-record-badge">🏆 Rekor</span>' : ''}</div>
              <div class="challenge-history-date">${dateStr}</div>
            </div>
          </div>
          <div class="challenge-history-value" style="color: ${ch.color};">${display}</div>
        </div>
      `;
    }).join('');
  }
  ...
  modal.innerHTML = `
    <div class="modal-header">
      <div style="font-family: 'Outfit', sans-serif; font-size: 1.3rem; font-weight: 800;">📋 Geçmiş Denemeler</div>
      <button class="modal-close" onclick="closeChallengeHistory()">✕</button>
    </div>
    <div class="challenge-history-list">${rows}</div>
  `;
  overlay.classList.add('active');
}

function closeChallengeHistory() {
  document.getElementById('challenge-history-overlay').classList.remove('active');
}
...
window.saveTimerChallenge = saveTimerChallenge;
window.saveRepsChallenge = saveRepsChallenge;
window.adjustReps = adjustReps;
window.showChallengeHistory = showChallengeHistory;
window.closeChallengeHistory = closeChallengeHistory;
